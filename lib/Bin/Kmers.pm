...
=head3 new
    ...
    my $kmers = Bin::Kmers->new($shrub, $workDir, \@repRoles, %options);
...
Working directory in which the intermediate JSON file is stored or is to be stored.
...
sub new {
    my ($class, $shrub, $workDir, $repRoles, %options) = @_;
    # Compute the options.
    ...
    # Compute the save file name.
    my $kmerFile = "$workDir/rep_kmers.json";
    ...
    if ($force || ! -f $kmerFile) {
        ...
        # Save the resulting kmer hash.
        SeedUtils::write_encoded_object(\%outHash, $kmerFile);
    }
    # Create the object.
    my $retVal = {
        fileName => $kmerFile,
...
sub FindGenomes {
    ...
    my %hits;
    # Read the kmer hash.
    my $kmerHash = SeedUtils::read_encoded_object($self->{fileName});
