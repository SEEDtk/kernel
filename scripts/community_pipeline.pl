...
sub Process {
    ...
    close PARMS;
    my $cmd = "initial_estimate -b $parms->{blast} -r $dataS/RefD -c $dataS/ref.counts -l $parms->{minlen} -p $parms->{maxpsc} -s $parms->{minsim} $norm -v $dataS/saved.$parms->{blast}.sim.vecs --cr $parms->{covgRatio} --ul $parms->{univLimit} > $dataS/bins$realSuffix";
    &SeedUtils::run($cmd);
    &SeedUtils::run("summarize_bins < $dataS/bins$realSuffix > $dataS/bins.summary$realSuffix");
}