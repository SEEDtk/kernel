...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# Report-header line should be:
# sample_id  repgen_id  repgen_name  count
#
# Add switches '--no-header' and '--no-sample-id'
#-----------------------------------------------------------------------
...
Minimum required length of 100% overlap region between forward (left) and reverse (right) reads (Default: 25nt)

=item nostrict

Turn off requirement for "strict" overlap.
(Default is "strict": A 100% match is required over the full length of the overlap region)
...
my $opt = P3Utils::script_opts(
    ...
    [ 'probe|p=i',     'Minimum length of 100% read-overlap (D: 25nt)',         { default => 25  } ],
    [ 'nostrict',      'Do not require 100% match over full length of read-overlap (D: strict)',  ],
    [],
...
my $strict      = $opt->nostrict() ? q() : q(--strict);
