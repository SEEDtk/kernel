...
my @potential_orgs =
  ...
  map { chop; [ split( /\t/, $_ ) ] } <$ih>;
BlastUtils::verify_db( $contigF, 'n' );
...
sub pull_ref_contigs
{
    ...
    foreach my $tuple (@$potential_orgs)
    {
        ...
        if (! -s "$giD/blast.out.dna")
        {
            ...
            my @matches = BlastUtils::blastn("$giD/reference.contigs", $contigF, { dust => 'no', maxE => $maxE });
        ...
        if (! -s "$giD/blast.out.protein")
        {
            ...
            my @matches = BlastUtils::tblastn("$giD/reference.translations", $contigF, { maxE => $maxE });
