...
## NOTE: This next statement was failing because "Subsystem" was misspelled. This was the hardest bug to find.
...
foreach my $ss (@privileged_subsystems)
{
    ...
    my @fids = $shrub->GetFlat("Subsystem2Feature", 'Subsystem2Feature(from-link) = ?', [$ss], 'Subsystem2Feature(to-link)');
