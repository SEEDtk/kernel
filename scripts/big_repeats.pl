...
if ($opt->genome) {
    ...
    if ($opt->input) {
        ...
        $queryFile = gjo::BlastInterface::get_query(\*STDIN);
        if (! $queryFile) {
...
if ($opt->blastdb) {
    ...
    $blastDB = gjo::BlastInterface::get_db($queryFile, 'blastn', $FIG_Config::temp);
    if (! $blastDB) {
...
# percent identity.
my $matches = gjo::BlastInterface::blast($queryFile, $blastDB, 'blastn',
